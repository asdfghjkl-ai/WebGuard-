...
def get_graph_from_html(html_content,label):
    ...
    node_ids = sorted(G.nodes())  # 关键步骤：固定节点顺序
    graph = S2VGraph(
        g=G,
        label=label
    )
...
def dataprocess(urls,input_ids,input_types,input_masks):
    pad_size = 200
    tokenizer = ConvBertTokenizer.from_pretrained("convbert-base")
    for i, url in tqdm(enumerate(urls), total=len(urls), desc="遍历 urls"):
        x1 = url  # Replace with the column name in your CSV file where the text data is located
...
class myConvBertModel(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, token_type_ids, extract_features=False):
        print(type(input_ids), type(attention_mask), type(token_type_ids))
...
class FinalModel(nn.Module):
    ...
    def forward(self, batch_graph, input_ids, input_types, input_masks):
        ...
        # 多批次text修改
        text_embeddings = text_embeddings.repeat_interleave(ym_select_sub_num, dim=0)
...
def check_batch_graphs_nodes(batch_graphs):
    ...
    batch_err = 0
    while pos:
        if batch_graphs[pos-1].g.number_of_nodes() == 0 : batch_err = batch_err + 1#图没有节点
        pos = pos - 1
    if batch_err == 3:
        return True
    else :return False
...
def test_final_model(model, test_graphs, test_loader, device):
    ...
    if(len(unique_classes) == 2):
        ...
        # weighted_f1 = tm.F1Score(num_classes=2, average='weighted')(torch.tensor(y_pred), torch.tensor(y_true_flat))
        # 新版本多了task
    ...
    print(f'TPR@FPR=0.0001: {tpr_at_fpr_00001:.4f}')
    print(f'TPR@FPR=0.001: {tpr_at_fpr_0001:.4f}')
    ...
    plt.tight_layout()
    plt.show()
    np.save('finalModel_scores_phishload.npy', y_scores)  # 概率
    np.save('finalModel_labels_phishload.npy', y_true)  # 标签

    return metrics

def find_min_divisor(n,nn):
    n_nn_min = min(nn,n-nn)
    if min(n_nn_min,5)==0: return 2
    else :return min(n_nn_min,5)
...
def build_dom_graph(dom):
    ...
    current_id = 0 # 递增ID
    pos2id = {}
    for elem in dom.find('*'):
        elem_data = elem.nodeList[0]
        pos2id[elem_data.pos] = current_id
        if not elem_data.nodeName in tags:
            mapped = len(tags)
            tags[elem_data.nodeName] = mapped
        node_tags.append(tags[elem_data.nodeName])
        node_attributes[current_id] = {
            'tag': elem_data.nodeName,  # 标签类型
            'attributes': str(elem_data.attributes),
            'text': elem_data.text
        }
        G.add_node(current_id)
        parent = elem_data.parentNode
        if parent is not None:
            G.add_edge(pos2id[parent.pos], pos2id[elem_data.pos])
...
true_graph_num = 0
bad_graph_num = 0
pickle_folder = 'path_pkl'
pickle_files = [f for f in os.listdir(pickle_folder) if f.endswith('.pickle')]
for file_name in pickle_files:
    ...
    with open(file_path, 'rb') as f:
        ...
        if isinstance(data, list) and len(data) > 0:
            ...
            print(f"Content of first element: {data[0]}")
        for url, attributes in data.items():
            dom = attributes.get('dom', None)
            status = attributes.get('status', '')
            src = attributes.get('src', '')
            label = 1 if src.lower() in ['phishtank', 'openphish'] else 0
            G, node_attributes, node_tags = build_dom_graph(dom)
...
dataprocess(urls,input_ids,input_types,input_masks)

val_data = TensorDataset(torch.tensor(input_ids).to(device),
...
metrics = test_final_model(model, graphs, val_loader, device)
