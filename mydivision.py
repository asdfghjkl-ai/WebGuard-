...
def graph_structure_division(graph, features_func,hash_func,num_group):
    """
    ...
    features = features_func(graph)
    edge_hash = get_edge_hash(graph.g, hash_func, features)
    # Divide the graph based on the edge hash values
    group = np.array([h % num_group for h in edge_hash])
...
def graph_feature_division(graph, features_func,hash_func,num_group):
    """
    ...
    group = np.array([h % num_group for h in node_hash])
    # Create subgraphs for each group
    subgraphs = []
    for i in range(num_group):
        subgraph = copy(graph)
...
def graph_node_division(graph, features_func,hash_func,num_group):
    """
    Divide a graph into subgraphs based on their NODE hash values
...
def graph_feature_structure_division(graph, features_func,hash_func,num_group):
    """
    ...
    graphs = graph_structure_division(graph)
...
def graph_edge_blank(graph, num_group):
    """
    ...
    return [graph] * num_group


def graph_node_blank(graph, num_group):
    """
    Divide a graph into subgraphs without edge
    ...
    return [subgraph] * num_group


def graph_blank(graph,num_group):
    """
    Divide a graph into subgraphs without edge
