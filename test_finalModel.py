...
def separate_data(graph_list, input_ids, input_types, input_masks, labels, seed, fold_idx, n=3):
    ...
    assert len(graph_list) == len(input_ids) == len(input_types) == len(input_masks) == len(
        labels), "All input lists must have the same length."
...
class myConvBertModel(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, token_type_ids, extract_features=False):
        ...
        text_embeddings = compressed_feature_tensor
...
def train_final_model(model, train_graphs, train_loader, criterion, optimizer, device):
    ...
    for idx, (train_input_ids, train_input_masks, train_input_types, train_labels) in enumerate(train_loader):
        train_input_ids, train_input_masks, train_input_types, train_labels = train_input_ids.to(
            device), train_input_masks.to(device), train_input_types.to(device), train_labels.to(device)
...
def test_final_model(model, test_graphs, test_loader, device):
    ...
    with torch.no_grad():
        for idx, (test_input_ids, test_input_masks, test_input_types, test_labels) in enumerate(test_loader):
            test_input_ids, test_input_masks, test_input_types, test_labels = test_input_ids.to(
                device), test_input_masks.to(device), test_input_types.to(device), test_labels.to(device)
    ...
    if (len(unique_classes) == 2):
        ...
        weighted_f1 = tm.F1Score(task='binary', num_classes=2, average='weighted')(torch.tensor(y_pred),
                                                                                   torch.tensor(y_true_flat))
    ...
    plt.tight_layout()
    plt.show()
    np.save('finalModel_scores.npy', y_scores)  # 概率
    np.save('finalModel_labels.npy', y_true)  # 标签
    return metrics
...
csv_file_path = "your_path"  # 替换为你的 CSV 文件路径
